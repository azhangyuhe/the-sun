...
class WebPage:
    ...
    @classmethod
    def new_from_url(cls, url: str, html, headers, verify: bool = True, timeout: Union[int, float] = 10):
        # response = requests.get(url, verify=verify, timeout=timeout)
        return cls.new_from_response(url, html, headers)
    ...
    @classmethod
    def new_from_response(cls, url, html, headers):
        return cls(url, html=html, headers=headers)
...
class Wappalyzer:
    ...
    def get_confidence(self, app_name):
        ...
        """
        return [] if 'confidenceTotal' not in self.technologies[app_name] else self.technologies[app_name][
            'confidenceTotal']
