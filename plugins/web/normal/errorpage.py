...
class Scan(Plugin):
    ...
    def check(self, html):
        ...
        for error in errors:
            regex = error["regex"]
            _type = error["type"]
            result = re.search(regex, html, re.I)
            if result:
                return _type, result.group()

    def scan(self, scanner):
        def random_name():
            name_len = random.randint(4, 10)
            tmp = random.choices(ascii_letters, k=name_len)
            return ''.join(tmp)

        headers = scanner.headers
        url = "{}://{}/".format(scanner.scheme, scanner.domain) + random_name() + ".jsp"
        response = self.my_get(url, headers)
        html = get_html(response)
        messages = self.check(html)
        if messages:
            _type, result = messages
            scanner.leak.append(
                {'name': 'error_page', 'more_info': {'payload': url, 'content': f":{_type}={result}"}})
